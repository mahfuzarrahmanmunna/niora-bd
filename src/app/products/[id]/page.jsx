...
const ProductDetails = () => {
  ...
  const getUserId = () => {
    ...
    if (session?.user) {
      return session.user.id || session.user._id || session.user.email;
    }
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // 2. Also save to API if we have a userId
      if (userId) {
        try {
          ...
          // Continue even if API fails - localStorage is our backup
        }
      }
  ...
  const handleAddRelatedToCart = async (e, relatedProduct) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    try {
      ...
      showNotification(`${relatedProduct.name} added to cart!`, "success");
    } catch (err) {
      console.error("Error adding to cart:", err);
      showNotification("Failed to add to cart", "error");
    }
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <button
                ...
                className="absolute bottom-4 right-4 bg-white rounded-full p-2 shadow-md hover:scale-110 transition-transform"
              >
                <Heart
                  className={`h-5 w-5 ${isWishlist ? "fill-red-500 text-red-500" : "text-gray-700"}`}
                />
              </button>

              <button className="absolute bottom-4 left-4 bg-white rounded-full p-2 shadow-md hover:scale-110 transition-transform">
                      ...
                      reviews.map((review) => (
                        ...
                        >
                          <div className="flex items-start space-x-4">
                            <Image
                              src={
                                review.profileImage || "/placeholder-avatar.png"
                              }
                              alt={review.name}
                              width={48}
                              height={48}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          onClick={(e) =>
                            handleAddRelatedToCart(e, relatedProduct)
                          }
                        >
