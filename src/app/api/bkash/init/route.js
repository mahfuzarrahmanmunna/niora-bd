...
export async function POST(request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Payment gateway configuration error. bKash credentials are missing.",
        },
        { status: 500 },
      );
    }
    ...
    if (!tokenData.id_token) {
      ...
      // Return the ACTUAL error message from bKash to the frontend
      return NextResponse.json(
        {
          success: false,
          message: tokenData.statusMessage || "Authentication failed",
          errorCode: tokenData.statusCode,
          details: tokenData,
        },
        { status: 500 },
      );
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            paymentResult.statusMessage || "Payment initialization failed",
        },
        { status: 400 },
      );
    }
